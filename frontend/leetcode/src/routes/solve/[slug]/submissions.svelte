<script context="module">
	...
	export async function load({ page }) {
		...
		const url = `https://goleetcode.xyz:8090/backend/submissions`;
		...
		if (Object.keys($submitCodeStore).length !== 0) {
			...
			try {
				const res = await fetch(
					`https://goleetcode.xyz:8090/backend/check/${$submitCodeStore.slug}`,
					options
				);
				const data = await res.json();
		...
		<div class="w-full">
			<table class="table-fixed items-center w-full border-collapse">
				<thead>
					<tr>
						<th
							class="w-1/5 px-4 py-2 bg-gray-200 border border-solid border-gray-100 border-r-0 text-sm text-gray-700 text-left"
							>Result</th
						>
						<th
							class="w-4/5 px-4 py-2 bg-gray-200 border border-solid border-gray-100 border-l-0 text-sm text-gray-700 text-left"
							>Output</th
						>
					</tr>
				</thead>
				{#each submissions as submission}
					<tbody>
						<tr>
							<td
								class="px-4 py-2 text-sm text-left {submission.result === 'OK'
									? 'text-green-600'
									: 'text-red-600'}"
							>
								{submission.result}
