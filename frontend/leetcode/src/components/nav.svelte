...
<nav class="flex justify-start w-full bg-gray-800 py-2 items-center fixed">
    <a href="/" on:click={() => {currentPage = "home"}}>
        <p class="mx-4 px-3 py-2 text-lg text-white font-extrabold">go-leetcode</p>
    </a>
    ...
    {#if $accessTokenStore != ""}
        <a href="/profile" on:click={() => {currentPage = "profile"}} class="absolute right-0 mx-4 px-3 py-2 rounded-md text-lg text-white font-medium {currentPage === "profile" ? "bg-gray-900" : "hover:bg-gray-700"}">Welcome, {username}</a>
    {:else}
