<script>
	...
	let comments = [];
	onMount(async () => {
		const url = `https://goleetcode.xyz/backend/discussions/${discussion.slug}/${discussion.id}`;
		const options = {
			method: 'GET'
		};
	...
	const postComment = async () => {
		...
		const url = `https://goleetcode.xyz/backend/discussions/${discussion.slug}/${discussion.id}/newcomment`;
