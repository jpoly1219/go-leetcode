...
func ProblemsetsAll(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
	...
	w.Header().Set("Access-Control-Allow-Origin", "http://44.207.2.100:3000")
	json.NewEncoder(w).Encode(problems)
}

func ProblemsetsFilter(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
	...
	if f.Difficulty == "all" {
		...
		results, err := models.Db.Query(
			"SELECT DISTINCT problem_id, title, problems.slug, difficulty, result FROM problems LEFT JOIN attempts ON problems.slug = attempts.slug AND username = $1 AND result = 'OK' WHERE difficulty = $2 ORDER BY title;",
...
func SolveSlug(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
...
func CheckSlug(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
	...
	resp, err := http.Post("http://44.207.2.100:8091/run", "application/json", responseBody)
	if err != nil {
...
func SolutionsSlug(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
...
func DiscussionsSlug(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
...
func UsersUsername(w http.ResponseWriter, r *http.Request) {
	auth.HandleCors(w, r)
	if r.Method == "OPTIONS" {
		return
	}
